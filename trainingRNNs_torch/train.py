...
def grad_time_profile(task, model, x: torch.Tensor, y_onehot: torch.Tensor, collect_extras: bool = False):
    ...
    # Compute activation derivative safely (handles both RNN and GRU)
    if hasattr(model, "act_deriv_from_h"):
        a_t = model.act_deriv_from_h(h).mean(dim=(1, 2))
    else:
        # Fallback for GRU where the activation is implicitly tanh
        a_t = (1.0 - h ** 2).mean(dim=(1, 2))
    ...
    return (
        loss.detach(),
        err.detach() if isinstance(err, torch.Tensor) else torch.tensor(err),
        g_t.detach(),
        a_t.detach(),
        sat_t.detach(),
